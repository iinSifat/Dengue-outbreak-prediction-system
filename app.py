...
def main():
    ...
    region = st.sidebar.selectbox(
        "Select Region",
        ["All Regions", "Dhaka", "Chittagong", "Sylhet", "Khulna", "Rajshahi"]
    )
    ...
    try:
        ...
        with tab1:
            ...
            fig = viz.plot_dengue_trend(
                processed_df,
                title=f"Dengue Cases - {region} (2020-2024)"
            )
            st.pyplot(fig)
            plt.close()
        ...
        with tab2:
            ...
            # Future predictions
            st.markdown("---")
            st.subheader(f"Predictions for {month} {year}")
            ...
            if target_prediction_index < len(monthly_predictions):
                ...
                with col1:
                    # Plot all future predictions with highlight
                ...
                with col2:
                    st.metric("Predicted Cases", f"{int(target_prediction)}")
                    st.metric("Risk Level", target_risk)
                    
                    # Risk alert
                    if target_prediction > 400:
                        st.markdown(
                            '<div class="risk-box high-risk">⚠️ HIGH RISK<br>{:.0f} cases</div>'.format(target_prediction),
                            unsafe_allow_html=True
                        )
                    elif target_prediction > 100:
                        st.markdown(
                            '<div class="risk-box medium-risk">⚡ MEDIUM RISK<br>{:.0f} cases</div>'.format(target_prediction),
                            unsafe_allow_html=True
                        )
                    else:
                        st.markdown(
